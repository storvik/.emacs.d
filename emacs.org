...
* Installation notes
- Symlink emacs.d to home folder
...
#+begin_src emacs-lisp
  ...
  (defmacro sys-diversification (darwin &optional gnu/linux win)
     (cond ((sys-type-p 'darwin) darwin)
           ((sys-type-p 'gnu/linux) gnu/linux)
...
** Initial settings
Set up =use-package= and add MELPA as archive.
...
#+begin_src emacs-lisp
  (setq backup-directory-alist
        `((".*" . ,(concat homedir-path "tmp/emacs.saves/"))))
  (setq auto-save-file-name-transforms
        `((".*" ,(concat homedir-path "tmp/emacs.saves/") t)))
 #+end_src
...
#+begin_src emacs-lisp
  ...
  ;;(set-face-attribute 'default nil :font "Fira Mono for Powerline-10")
#+End_src

** Golden ratio
[[https://github.com/roman/golden-ratio.el][Visit website]].
Resize windows so that the active window is optimized for editing.
Todo: Use (x-display-pixel-width) and (x-display-pixel-height) to figure out if golden ratio should be used.
...
** Line numbering
Show line numbers on left side and include linum-off from ~/emacs-d/elisp/.
...
** Typit
Typing game to learn faster typing / touch typing.
...
Export org files as reveal.js presentations.

#+begin_src emacs-lisp
  (use-package ox-reveal
    :ensure ox-reveal)

  (setq org-reveal-root "https://cdnjs.cloudflare.com/ajax/libs/reveal.js/3.4.0/")
