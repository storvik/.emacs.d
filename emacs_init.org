...
** Keybindings
All overrides are [[https://stackoverflow.com/questions/683425/globally-override-key-binding-in-emacs][placed in a minor mode]].
This way it's easy to completely disable all overrides if default behaviour is needed.
Other custom keybindings are placed behind a prefix map, making it easy to maintain and discover using whick-key.

Adding keybindings to map can be done the following ways:

: (bind-key "m w" #'function-name my-map)
...
*** My key overrides minor mode

#+begin_src emacs-lisp
  (defvar my-overrides-minor-mode-map
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "M-o") 'other-window)
      (define-key map (kbd "RET") 'newline-and-indent)
      map)
    "my-overrides-minor-mode keymap.")

  (define-minor-mode my-overrides-minor-mode
    "A minor mode so that my key settings override major modes."
    :init-value t
    :lighter " my-overrides")

  ;; Enable minor mode, maybe it should
  (my-overrides-minor-mode 1)

  ;; Disable overrides minor mode in minibuffer
  (defun my-overrides-minibuffer-setup-hook ()
    (my-overrides-minor-mode 0))

  (add-hook 'minibuffer-setup-hook 'my-overrides-minibuffer-setup-hook)
#+end_src
...
#+begin_src emacs-lisp
  (use-package multiple-cursors
    :bind (:map my-map ("c" . hydra-multiple-cursors/body))
    :config
...
#+begin_src emacs-lisp
  (use-package ace-window
    :bind (:map my-overrides-minor-mode-map ("C-x o" . ace-window))
    :init
...
#+begin_src emacs-lisp
  (use-package avy
    :bind (:map my-map (";" . hydra-avy/body)
           :map my-overrides-minor-mode-map ("M-g g" . avy-goto-line))
    :config
...
#+begin_src emacs-lisp
  (use-package expand-region
    :bind (:map my-map ("e" . hydra-expand-region/body))
    :config
...
#+begin_src emacs-lisp
  (use-package dumb-jump
    :bind (:map my-map ("d" . hydra-dumb-jump/body))
    :config
...
#+begin_src emacs-lisp
  (use-package move-text
    :bind (:map my-map ("m" . hydra-move-text/body))
    :config
...
#+begin_src emacs-lisp
  ...
  (bind-key "n" #'narrow-or-widen-dwim my-map)
#+end_src
...
#+begin_src emacs-lisp
  (use-package ryo-modal
    ...
    :bind (:map my-map ("SPC" . ryo-modal-mode))
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    :bind (:map my-map ("g" . magit-status))
    :config
