...
#+begin_src emacs-lisp
  (use-package svg-tag-mode
    :hook ((prog-mode . svg-tag-mode)
           (org-mode . storvik/svg-tag-org-mode-setup))
    :custom
    (svg-tag-tags
    ...
    :init
    (defconst storvik/date-re "[0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\}")
    (defconst storvik/time-re "[0-9]\\{2\\}:[0-9]\\{2\\}")
    (defconst storvik/day-re "[A-Za-z\\|ø.]*")
    (setq storvik/svg-tag-org-mode-tags
          `(("TODO\\b" . ((lambda (tag) (svg-tag-make "TODO" :face 'org-done :inverse t :margin 0))))
            ...
            (,(format "\\(MEETING\\)\\W<%s\\W%s\\W%s>" storvik/date-re storvik/day-re storvik/time-re) .
            ...
            (,(format "MEETING\\W<%s\\W%s\\W%s>\\(\\W.*\\)" storvik/date-re storvik/day-re storvik/time-re) .
             ((lambda (tag) (svg-tag-make tag :face 'org-todo :crop-left t))))
            ))
    :config
    (defun storvik/svg-tag-org-mode-setup ()
      "org-mode setup hook which sets up svg-tag-mode"
      (interactive)
      (setq-local svg-tag-tags storvik/svg-tag-org-mode-tags)
      (svg-tag-mode))
