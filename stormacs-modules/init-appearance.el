...
(use-package dashboard
  ...
  (defun stormacs-dashboard-insert-init-info-svg ()
    ...
    (let ((start (point)))
      ...
      (insert-image (svg-tag-make (shell-command-to-string "echo -n $( mu find maildir:/petter@storvik.dev/Inbox flag:unread 2>/dev/null | wc -l )")
                                  :inverse nil :crop-left t :margin 1))
      (insert "   ")
      (insert-image (svg-tag-make "svartisenfestivalen" :inverse t :crop-right t :margin 0))
      (insert-image (svg-tag-make (shell-command-to-string "echo -n $( mu find maildir:/petter@svartisenfestivalen.no/Inbox flag:unread 2>/dev/null | wc -l )")
                                  :inverse nil :crop-left t :margin 1))
      (insert "   ")
      (insert-image (svg-tag-make "tiveinc" :inverse t :crop-right t :margin 0))
      (insert-image (svg-tag-make (shell-command-to-string "echo -n $( mu find maildir:/petter.storvik@tive.com/Inbox flag:unread 2>/dev/null | wc -l )")
                                  :inverse nil :crop-left t :margin 1))
